...
func construct(ctx *pulumi.Context, typ, name string, inputs provider.ConstructInputs,
	...
	switch typ {
	case "pulumi-aws-vpc:index:PulumiAwsVpc":
		return newPulumiAwsVpc(ctx, name, inputs, options)
	default:
...
func newPulumiAwsVpc(ctx *pulumi.Context, name string, inputs provider.ConstructInputs,
	options pulumi.ResourceOption) (*provider.ConstructResult, error) {
	...
	awsVpc, err := NewPulubmiAwsVpc(ctx, name, args, options)
	if err != nil {
	...
	return provider.NewConstructResult(awsVpc)
}
